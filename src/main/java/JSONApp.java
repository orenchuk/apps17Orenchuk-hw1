...
public class JSONApp {
    ...
    public static JsonObject sessionResult() {
        ...
        JsonPair OOPpassed = new JsonPair("passed", new JsonBoolean(true));

        JsonPair EnglishCourse = new JsonPair("course", new JsonString("English"));
        JsonPair EnglishMark = new JsonPair("mark", new JsonNumber(5));
        JsonPair EnglishPassed = new JsonPair("passed", new JsonBoolean(true));

        JsonPair MathCourse = new JsonPair("course", new JsonString("Math"));
        JsonPair MathMark = new JsonPair("mark", new JsonNumber(2));
        JsonPair MathPassed = new JsonPair("passed", new JsonBoolean(false));

        JsonObject oop = new JsonObject(OOPcourse, OOPmark, OOPpassed);
        JsonObject english = new JsonObject(EnglishCourse, EnglishMark, EnglishPassed);
        JsonObject math = new JsonObject(MathCourse, MathMark, MathPassed);
